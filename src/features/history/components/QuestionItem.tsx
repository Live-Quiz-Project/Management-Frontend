      ...
      const buildChartTypesButton = () => {
        return (
          <div className="flex p-0.5 rounded-sm bg-pastel-orange">
            <button className={`rounded-sm px-2 ${chartType === 'BarChart' ? 'bg-white' : null } mr-1`} onClick={() => setChartType("BarChart")}>
              <BarChartIcon style={{ fontSize: 18 }}/>
            </button>
            <button className={`rounded-sm px-2 ${chartType === 'PieChart' ? 'bg-white' : null }`} onClick={() => setChartType("PieChart")}>
